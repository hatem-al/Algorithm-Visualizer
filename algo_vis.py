...
def draw(draw_info, algo_name, ascending, speed):
    ...
    controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Descending | +/- Speed", 1, draw_info.WHITE)
    draw_info.window.blit(controls, (draw_info.width/2 - controls.get_width()/2 , 45))

    sorting = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort", 1, draw_info.WHITE)
    draw_info.window.blit(sorting, (draw_info.width/2 - sorting.get_width()/2 , 75))

    speed_display = draw_info.FONT.render(f"Speed: {speed}", 1, draw_info.WHITE)
    draw_info.window.blit(speed_display, (draw_info.width/2 - speed_display.get_width()/2, 105))
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            if event.key == pygame.K_r:
                ...
                sorting_algorithm = bubble_sort
                sorting_algo_name = "Bubble Sort"
            elif event.key == pygame.K_PLUS or event.key == pygame.K_EQUALS:  # Increase speed
                speed = min(speed + 10, 240)  # Limit max speed
            elif event.key == pygame.K_MINUS:  # Decrease speed
                speed = max(speed - 10, 10)  # Limit min speed
